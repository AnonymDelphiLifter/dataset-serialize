...
{$ENDIF}
  ...
  private
    ...
    /// </remarks>
    function DataSetToJSONObject(const ADataSet: TDataSet;
    ...
    function DataSetToJSONArray(const ADataSet: TDataSet;
      const IsChild: Boolean; const IsValue: Boolean = True;
      const IsEncodeBlob: Boolean = True): TJSONArray;
    /// <summary>
...
{$ELSE}
  System.DateUtils, Data.FmtBCD, System.SysUtils, System.StrUtils,
  System.TypInfo, System.Classes, System.NetEncoding,
...
begin
  if TDataSetSerializeConfig.GetInstance.&Export.ExportFloatScientificNotation
  then
  ...
  begin
    ...
    if (TDataSetSerializeConfig.GetInstance.&Export.DecimalSeparator <> '') then
      LFormatSettings.DecimalSeparator :=
        TDataSetSerializeConfig.GetInstance.&Export.DecimalSeparator;
      ...
      begin
        case ADataSet.Fields[0].DataType of
          ...
          TFieldType.ftSingle, TFieldType.ftFloat:
            begin
              if TDataSetSerializeConfig.GetInstance.Export.FormatFloat.Trim.IsEmpty
              then
                Result.Add(ADataSet.Fields[0].AsFloat)
              else
                Result.Add
                  (FormatFloat(TDataSetSerializeConfig.GetInstance.Export.
                  FormatFloat, ADataSet.Fields[0].AsFloat));
            end;
          ...
          TFieldType.ftGraphic, TFieldType.ftBlob, TFieldType.ftOraBlob,
            TFieldType.ftOraClob, TFieldType.ftStream:
            begin
...
begin
  ...
  begin
    ...
    begin
      Result.{$IF DEFINED(FPC)}Add{$ELSE}AddPair{$ENDIF}(LKey,
        TJSONString.Create(LField.Text));
      Continue;
    end;
    case LField.DataType of
      ...
      TFieldType.ftInteger, TFieldType.ftSmallint,
        TFieldType.ftAutoInc{$IF NOT DEFINED(FPC)}, TFieldType.ftShortint,
        TFieldType.ftLongWord, TFieldType.ftWord, TFieldType.ftByte{$ENDIF}:
        Result.{$IF DEFINED(FPC)}Add{$ELSE}AddPair{$ENDIF}(LKey,
{$IF DEFINED(FPC)}LField.AsInteger{$ELSE}TJSONNumber.Create
          (LField.AsInteger){$ENDIF});
      TFieldType.ftLargeint:
        begin
          if TDataSetSerializeConfig.GetInstance.Export.ExportLargeIntAsString
          then
            Result.{$IF DEFINED(FPC)}Add{$ELSE}AddPair{$ENDIF}(LKey,
{$IF DEFINED(FPC)}LField.AsLargeInt.ToString{$ELSE}TJSONString.
              Create(LField.AsLargeInt.ToString){$ENDIF})
          else
            Result.{$IF DEFINED(FPC)}Add{$ELSE}AddPair{$ENDIF}(LKey,
...
{$ENDIF}TFieldType.ftFloat:
        begin
          ...
          then
            Result.{$IF DEFINED(FPC)}Add{$ELSE}AddPair{$ENDIF}(LKey,
{$IF DEFINED(FPC)}TJSONExtFloatNumber.Create
              (LField.AsFloat){$ELSE}TJSONNumber.Create(LField.AsFloat){$ENDIF})
          else
            Result.{$IF DEFINED(FPC)}Add{$ELSE}AddPair{$ENDIF}(LKey,
              TJSONString.Create
              (FormatFloat(TDataSetSerializeConfig.GetInstance.Export.
      ...
      TFieldType.ftString, TFieldType.ftWideString, TFieldType.ftMemo,
        TFieldType.ftWideMemo, TFieldType.ftFixedChar,
        TFieldType.ftFixedWideChar:
        begin
          LStringValue := Trim(LField.AsWideString);
          if (LStringValue = EmptyStr) and
            (TDataSetSerializeConfig.GetInstance.Export.ExportEmptyStringAsNull)
          then
            Result.{$IF DEFINED(FPC)}Add{$ELSE}AddPair{$ENDIF}(LKey,
              TJSONNull.Create())
          else if TDataSetSerializeConfig.GetInstance.Export.TryConvertStringToJson
          then
          begin
            ...
            begin
              try
                Result.{$IF DEFINED(FPC)}Add{$ELSE}AddPair{$ENDIF}(LKey,
{$IF DEFINED(FPC)}GetJSON(LStringValue){$ELSE}TJSONObject.
                  ParseJSONValue(LStringValue){$ENDIF});
              except
                Result.{$IF DEFINED(FPC)}Add{$ELSE}AddPair{$ENDIF}(LKey,
                  TJSONString.Create(LField.AsWideString));
              end;
      ...
      TFieldType.ftCurrency:
        begin
          ...
          then
            Result.{$IF DEFINED(FPC)}Add{$ELSE}AddPair{$ENDIF}(LKey,
{$IF DEFINED(FPC)}TJSONExtFloatNumber.Create
              (LField.AsCurrency){$ELSE}TJSONNumber.Create
      ...
      TFieldType.ftFMTBcd, TFieldType.ftBCD:
        if TDataSetSerializeConfig.GetInstance.Export.ExportBCDAsFloat then
          Result.{$IF DEFINED(FPC)}Add{$ELSE}AddPair{$ENDIF}(LKey,
{$IF DEFINED(FPC)}TJSONExtFloatNumber.Create
            (BcdToDouble(LField.AsBcd)){$ELSE}TJSONNumber.Create
...
{$ENDIF}
      TFieldType.ftGraphic, TFieldType.ftBlob, TFieldType.ftOraBlob,
        TFieldType.ftOraClob{$IF NOT DEFINED(FPC)}, TFieldType.ftStream{$ENDIF}:
        Result.{$IF DEFINED(FPC)}Add{$ELSE}AddPair{$ENDIF}(LKey,
          TJSONString.Create(IfThen(FEncodeBase64Blob,
          EncodingBlobField(LField), LField.AsString)));
      TFieldType.ftVarBytes, TFieldType.ftBytes:
        begin
          LHexString := EmptyStr;
...
{$ENDIF}
        try
          if (not TDataSetSerializeConfig.GetInstance.Export.ExportEmptyDataSet)
            and (LNestedDataSet.RecordCount = 0) then
            Continue;
          if TDataSetSerializeConfig.GetInstance.Export.ExportOnlyFieldsVisible
            and (not HasVisibleFields(LNestedDataSet)) then
            Continue;
          if string(LNestedDataSet.Name).Trim.IsEmpty then
          begin
            Inc(LDataSetNameNotDefinedCount);
            LDataSetName := TDataSetSerializeUtils.FormatDataSetName
              ('dataset_name_not_defined_' +
